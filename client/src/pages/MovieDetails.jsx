...
const MovieDetails = () => {
  ...
  const handleFavorite = async()=>{
   ...
   // 👇 New function to open YouTube trailer
  const handleWatchTrailer = () => { 
    if (!show?.movie) return  

    // If API already provides trailer info
    if (show.movie.videos && show.movie.videos.results.length > 0) {
      const trailer = show.movie.videos.results.find(
    ...
    const searchQuery = encodeURIComponent(`${show.movie.title} official trailer`)
